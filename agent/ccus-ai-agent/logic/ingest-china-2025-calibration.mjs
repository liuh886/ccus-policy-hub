...
async function ingest() {
  ...
  try {
    ...
    db.run(
      `INSERT OR REPLACE INTO policies (id, country, year, status, category, review_status, legal_weight, source, pub_date, provenance_author, provenance_last_audit_date) VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
      [
        demoId,
        'China',
        2024,
    ...
    db.run(
      `INSERT OR REPLACE INTO policy_i18n (policy_id, lang, title, description, impact_analysis_json) VALUES (?,?,?,?,?)`,
      [
        demoId,
        'en',
    ...
    db.run(
      `INSERT OR REPLACE INTO policy_i18n (policy_id, lang, title, description) VALUES (?,?,?,?)`,
      [
        roadmapId,
        'en',
        'China CCUS Technology Development Roadmap (2025)',
        'Official 2025 update shifting strategic focus from technical verification to large-scale commercial application and hub-based clusters.',
      ]
    );
    ...
    db.run(
      ...
      [
        standardsId,
        'en',
        'Batch Release of CCUS National Standards (2024)',
    ...
    db.run(
      `UPDATE policy_i18n SET description = ?, impact_analysis_json = ? WHERE policy_id = 'cn-ccer' AND lang = 'en'`,
      [
        ...
        JSON.stringify({
          economic:
            'Projected market size of 73.5B CNY by 2030; CEA prices >100 RMB/t.',
    ...
    db.run(
      `UPDATE policy_i18n SET description = ?, impact_analysis_json = ? WHERE policy_id = 'cn-ccer' AND lang = 'zh'`,
      [
    ...
    const pore_en =
      'State-owned; 2024 decoupling from mineral rights initiated; Sea area use rights integrated for offshore.';
    const pore_zh =
      '国家所有；2024 年启动封存权与矿权剥离试点；海上项目已整合海域使用权。';
    const liab_en =
      'Staged Responsibility Model (20-30y monitoring); Financial Assurance Fund under development (2025).';
    const liab_zh =
      '阶段性责任模型（20-30 年监测期）；2025 年启动国家财务保证基金建设。';
    const period_en = 'Draft 2025 standards suggest 20-30 years post-closure.';
    const period_zh = '2025 年标准草案建议封存后监测期为 20-30 年。';

    db.run(
      `UPDATE country_i18n SET pore_space_rights = ?, liability_transfer = ?, liability_period = ? WHERE country_id = 'China' AND lang = 'en'`,
      [pore_en, liab_en, period_en]
    );
    db.run(
      `UPDATE country_i18n SET pore_space_rights = ?, liability_transfer = ?, liability_period = ? WHERE country_id = 'China' AND lang = 'zh'`,
