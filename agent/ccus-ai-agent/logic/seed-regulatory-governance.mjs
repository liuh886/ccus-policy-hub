...
async function run() {
  ...
  try {
    for (const [policyId, data] of Object.entries(GOLDEN_RECORDS)) {
      ...
      db.run(`UPDATE policy_i18n SET regulatory_json = ? WHERE policy_id = ?`, [
        json,
        policyId,
      ]);
      ...
      const countryRow = db.exec(
        `SELECT id FROM country_profiles WHERE id = '${data.country}'`
      );
      if (countryRow.length > 0 && countryRow[0].values.length > 0) {
        ...
        ['en', 'zh'].forEach((lang) => {
          db.run(
            ...
            [
              r.pore_space_rights,
              r.liability_transfer,
              r.liability_period,
              r.financial_assurance,
              r.permitting_lead_time,
              r.co2_definition,
              r.cross_border_rules,
              cId,
              lang,
            ]
          );
