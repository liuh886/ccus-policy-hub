...
async function initStandard() {
  ...
  db.run(
    'CREATE TABLE IF NOT EXISTS dict_region_alias (en TEXT PRIMARY KEY, zh TEXT NOT NULL)'
  );
  db.run(
    'CREATE TABLE IF NOT EXISTS dict_source_alias (alias TEXT PRIMARY KEY, canonical TEXT NOT NULL)'
  );
  ...
  for (const [en, zh] of regions) {
    db.run('INSERT OR REPLACE INTO dict_region_alias (en, zh) VALUES (?, ?)', [
      en,
      zh,
    ]);
  }
