...
import {
  ...
  applyContentDepthBatch3D,
} from './content-depth-batch3d-2026-09.mjs';
...
test('content-depth batch 3D enriches five EU policies in memory (no save)', async () => {
  const db = await openDatabase();
  const summary = applyContentDepthBatch3D(db, { auditDate: '2026-09-09' });
  ...
  const aiMarkers = rows(
    ...
    `SELECT COUNT(*) AS n FROM policy_analysis
      WHERE policy_id IN ('be-ccus-strategy-2025','gr-ccs-regulatory-framework-2025','fr-ccus-roadmap','es-climate-law-ccs-2025','pl-mining-law-ccs-2024')
        AND (evidence LIKE '%[AI-Generated]%' OR label LIKE '%Initial Assessment%' OR evidence LIKE '%Batch 4 Offshore Focus update%' OR evidence LIKE '%Alignment with EU CCS Directive and industrial strategy%' OR evidence LIKE '%EU Directive alignment provides strong statutory basis%')`,
    []
  )[0].n;
...
test('batch 3D targets score >= 70 on the real audit scorer', async () => {
  const db = await openDatabase();
  applyContentDepthBatch3D(db, { auditDate: '2026-09-09' });
