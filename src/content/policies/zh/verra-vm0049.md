---
{
  ...
  'impactAnalysis':
    {
      'economic': '为符合条件的项目提供潜在自愿碳信用收入路径，但能否签发取决于注册、审定、核查和签发程序，而非方法学发布本身。',
      'environmental': '要求额外性、监测、泄漏处理、永久性保障和避免重复计算，以维护获签发减排或移除信用的完整性。',
      'technical': '通过模块化项目边界和量化程序处理不同CCS配置下的捕集、运输、封存、泄漏和项目排放。',
    },
  ...
  'regulatory':
    {
      'crediting_boundary': 'VM0049属于自愿碳信用方法学，VCU签发仍需完成项目注册、审定、核查，并符合现行模块和工具要求。',
      'hub_accounting': '当项目能够证明边界、交接链、权属及避免重复计算时，模块化结构可支持共享CCS枢纽核算。',
      'permitting_boundary': '采用该方法学不能替代所在国的捕集设施、二氧化碳运输、注入和地质封存许可。',
    },
  'relatedFacilities': [],
  'reviewStatus': 'verified',
  'source': 'Verra',
  'status': '现行',
  'title': 'Verra CCS 核证方法学 (VM0049)',
