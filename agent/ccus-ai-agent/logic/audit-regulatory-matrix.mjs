...
locales.forEach((lang) => {
  ...
  files.forEach((file) => {
    ...
    if (lang === 'en') {
      if (populatedCount === REG_FIELDS.length)
        results.summary.fully_populated++;
...
report +=
  '- Partially Populated: ' + results.summary.partially_populated + '\n';
report += '- Completely Empty: ' + results.summary.empty + '\n\n';

report += '## Coverage by Country\n\n';
report +=
  '| Country | Policies | ' +
  REG_FIELDS.map((f) => f.replace(/_/g, ' ')).join(' | ') +
  ' | Fill Rate |\n';
report +=
  '|---------|----------|' +
  REG_FIELDS.map(() => '---').join('|') +
...
Object.values(results.by_country)
  ...
  .forEach((c) => {
    const row = [
      ...
      c.policy_count,
      ...REG_FIELDS.map(
        (f) => Math.round((c.fields[f] / c.policy_count) * 100) + '%'
      ),
      Math.round(
        (Object.values(c.fields).reduce((a, b) => a + b, 0) /
          (c.policy_count * REG_FIELDS.length)) *
          100
      ) + '%',
    ];
    report += '| ' + row.join(' | ') + ' |\n';
