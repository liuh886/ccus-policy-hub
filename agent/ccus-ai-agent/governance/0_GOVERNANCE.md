...
所有数据变更必须严格对齐 `agent/ccus-ai-agent/db/schema.sql` 中的 Relational 定义以及 `src/content/config.ts` 中的 Zod 定义。
...
- **SSOT**：`agent/ccus-ai-agent/db/ccus_master.sqlite` 是唯一真理。
...
3.  **并发锁 (Concurrency Lock)**：通过 `agent/ccus-ai-agent/db/.lock` 机制确保同一时间只有一个治理进程操作 SSOT。
...
- **临时文件治理 (Temp File Management)**：
  ...
  - `agent/ccus-ai-agent/governance/reports/` 仅保留最新的审计日志与心跳文件。
...
- **文件数量治理**：`agent/ccus-ai-agent/governance/` 根目录只允许核心白名单文件；阶段性文档必须进入 `agent/ccus-ai-agent/governance/archive/`。
