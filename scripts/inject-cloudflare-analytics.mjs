...
for (const file of files) {
  ...
  if (!html.includes('</head>')) throw new Error(`Built HTML has no </head>: ${file}`);
  await writeFile(file, html.replace('</head>', `${beacon}</head>`), 'utf8');
  injected += 1;
}

console.log(`Cloudflare Web Analytics injected into ${injected} of ${files.length} built HTML files.`);
