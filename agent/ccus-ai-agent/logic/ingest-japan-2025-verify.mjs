...
async function ingest() {
  ...
  try {
    ...
    db.run(
      `UPDATE policies SET provenance_last_audit_date = ?, provenance_author = ? WHERE id = ?`,
    ...
    const reg_json = {
      ...
      liability_transfer:
        'Monitoring/Management responsibility transfers to JOGMEC after stability confirmation.',
      liability_period:
        'Phase-in from 2024-2026; monitoring contribution required during operation.',
      financial_assurance:
        'Mandatory Monitoring Reserves (Contributions) paid to JOGMEC.',
      permitting_lead_time:
        'Approx. 2-3 years via METI Specified Area licensing.',
      co2_definition:
        'Regulated industrial commodity with strict environmental safety standards.',
      cross_border_rules:
        'Aligned with London Protocol (Art 6) to enable Asia-Pacific hubs.',
    };

    const impact_json = {
      economic:
        'Enables project financing through real-right registration; reduces long-term liability costs via JOGMEC transfer.',
      technical:
        "Establishes 'Stable Behavior' as the legal threshold for monitoring transfer, driving 4D seismic adoption.",
      environmental:
        "Legal backbone for Japan's 2030 target of 6-12 Mtpa storage capacity.",
    };

    db.run(
      `UPDATE policy_i18n SET title = ?, description = ?, impact_analysis_json = ?, regulatory_json = ? WHERE policy_id = ? AND lang = ?`,
      [
        'Act on Carbon Dioxide Storage Business (2025 Phase-in)',
        desc_en,
        JSON.stringify(impact_json),
    ...
    const scores = [
      [
        'incentive',
        75,
        'Quasi-real right status enables bankability and mortgage financing.',
        'Art. 45 & Mina-shi Bukken',
      ],
      [
        'statutory',
        98,
        'Primary legislation with clear phased enforcement timeline.',
        'Full Act',
      ],
      [
        'market',
        80,
        'Open access mandate and real-right tradability.',
        'Chapter 4',
      ],
      [
        'strategic',
        95,
        'Cornerstone of GX strategy and Asia CCUS Network.',
        'METI Strategy',
      ],
      [
        'mrv',
        90,
        'Clear legal definition of "Stability" for JOGMEC transfer.',
        'Section 5',
      ],
    ];
    ...
    for (const s of scores) {
      db.run(
        `UPDATE policy_analysis SET score = ?, evidence = ?, citation = ? WHERE policy_id = ? AND dimension = ?`,
