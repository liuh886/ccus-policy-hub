...
import {
  ...
  applyContentDepthBatch3F,
} from './content-depth-batch3f-2026-09.mjs';
...
test('content-depth batch 3F enriches six SEA policies in memory (no save)', async () => {
  const db = await openDatabase();
  const summary = applyContentDepthBatch3F(db, { auditDate: '2026-09-09' });
  ...
  const aiMarkers = rows(
    ...
    `SELECT COUNT(*) AS n FROM policy_analysis
      WHERE policy_id IN ('my-netr-ccus-2023','my-mida-incentives','ph-ccus-policy-framework-2025','th-draft-climate-change-act-2025','sg-carbon-tax-ccus-2025','vn-carbon-market-decree-2025')
        AND (evidence LIKE '%[AI-Generated]%' OR label LIKE '%Initial Assessment%' OR evidence LIKE '%No direct incentive found%' OR evidence LIKE '%No specific market rules%' OR evidence LIKE '%Standard reporting required%' OR evidence LIKE '%Batch 4 Offshore Focus update%' OR evidence LIKE '%Medium-High | Significant legislative progress%' OR evidence LIKE '%Excellent | High carbon tax%')`,
    []
  )[0].n;
  assert.equal(aiMarkers, 0, 'no boilerplate markers remain');

  const thStatus = rows(db, 'SELECT status FROM policies WHERE id = ?', [
    'th-draft-climate-change-act-2025',
  ])[0].status;
  assert.equal(thStatus, 'Under development');
  db.close();
});

test('batch 3F targets score >= 70 on the real audit scorer', async () => {
  const db = await openDatabase();
  applyContentDepthBatch3F(db, { auditDate: '2026-09-09' });
