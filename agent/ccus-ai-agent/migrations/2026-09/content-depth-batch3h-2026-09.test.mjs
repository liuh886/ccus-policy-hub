...
import {
  ...
  applyContentDepthBatch3H,
} from './content-depth-batch3h-2026-09.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const DB_PATH = path.resolve(__dirname, '../../db/ccus_master.sqlite');

const TARGETS = [
  'eg-carbon-market-framework-2024',
  'kz-ecology-code-ccus-2024',
  'tr-climate-law-2025',
  'za-climate-change-act-2024',
  'ru-climate-doctrine-2023',
  'ng-nuprc-decarb-2024',
];
...
test('content-depth batch 3H enriches six policies in memory (no save)', async () => {
  const db = await openDatabase();
  const summary = applyContentDepthBatch3H(db, { auditDate: '2026-09-09' });
  ...
  const aiMarkers = rows(
    ...
    `SELECT COUNT(*) AS n FROM policy_analysis
      WHERE policy_id IN ('eg-carbon-market-framework-2024','kz-ecology-code-ccus-2024','tr-climate-law-2025','za-climate-change-act-2024','ru-climate-doctrine-2023','ng-nuprc-decarb-2024')
        AND (evidence LIKE '%[AI-Generated]%' OR label LIKE '%Initial Assessment%' OR evidence LIKE '%Established FRA carbon credit framework%' OR evidence LIKE '%Ecology Code provides robust legal basis%' OR evidence LIKE '%New framework law establishes TR ETS%' OR evidence LIKE '%First comprehensive upstream decarbonization framework in Africa%')`,
    []
  )[0].n;
...
test('batch 3H targets score >= 70 on the real audit scorer', async () => {
  const db = await openDatabase();
  applyContentDepthBatch3H(db, { auditDate: '2026-09-09' });
