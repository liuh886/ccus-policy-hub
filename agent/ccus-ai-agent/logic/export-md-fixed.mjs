async function dbExportMd(SQL) {
  ...
  if (!auditPass || auditPass.value !== 'true')
    throw new Error('Export blocked. Last audit failed. (Gate B2)');
  ...
  const translate = (key, domain, lang) => {
    ...
    if (domain === 'status' && dict.ui.status[key]?.zh)
      return dict.ui.status[key].zh;
    if (domain === 'category' && dict.ui.categories[key]?.zh)
  ...
  db.all('SELECT * FROM policies ORDER BY id').forEach((p) => {
    const analysis = {};
    db.all(
      'SELECT * FROM policy_analysis WHERE policy_id = ? ORDER BY dimension',
      [p.id]
    ).forEach((r) => {
      analysis[r.dimension] = {
        score: r.score || 0,
        label: r.label || 'Pending',
        evidence: r.evidence || 'No evidence provided yet.',
        citation: r.citation || '',
        auditNote: r.audit_note || '',
      };
    });
    const relatedFacs = db
      .all(
    ...
    ['en', 'zh'].forEach((lang) => {
      ...
      const frontmatter = deepClean({
        ...
        provenance: {
          ...
          reviewer: p.provenance_reviewer || 'Human Audit Pending',
...
This project is located in ${displayCountry}${i18n.region ? ` (${i18n.region})` : ''}, within the ${i18n.sector || 'CCUS'} sector. The facility is classified as ${i18n.type || 'Capture'} and is currently ${displayStatus}. ${i18n.hub ? `As part of the ${i18n.hub} hub, ` : ''}${i18n.operator ? `it is operated by ${i18n.operator}.` : ''}`;
      ...
      const frontmatter = deepClean({
        ...
        coordinates: [f.lat || 0, f.lng || 0],
        precision: f.precision || 'approximate',
        sector: i18n.sector || '',
        fateOfCarbon: i18n.fate_of_carbon || '',
        hub: i18n.hub || '',
        phase: i18n.phase || '',
        ...
        suspensionDate: i18n.suspension_date || '',
        partners: partners.sort(),
        links: links.filter((l) => l.startsWith('http')).sort(),
        relatedPolicies: relatedPols.sort(),
        provenance: {
  ...
  db.all('SELECT * FROM country_profiles ORDER BY id').forEach((c) => {
    ['en', 'zh'].forEach((lang) => {
      ...
      if (!fs.existsSync(contentDir))
        fs.mkdirSync(contentDir, { recursive: true });
      fs.writeFileSync(
        path.join(contentDir, `${c.id.toLowerCase().replace(/ /g, '-')}.md`),
        `---
${JSON.stringify(frontmatter, null, 2)}
---
