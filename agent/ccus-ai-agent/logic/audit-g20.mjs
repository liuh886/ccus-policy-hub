...
async function runAudit() {
  ...
  for (const country of G20_MEMBERS) {
    ...
    const profileStmt = db.prepare(
      'SELECT * FROM country_profiles WHERE id = ?'
    );
    ...
    const policyStmt = db.prepare(
      'SELECT COUNT(*) as count FROM policies WHERE country = ?'
    );
    ...
    const i18nStmt = db.prepare(
      "SELECT * FROM country_i18n WHERE country_id = ? AND lang = 'en'"
    );
  ...
  results.forEach((r) => {
    ...
    console.log(
      `${status} ${r.country.padEnd(17)} | Policies: ${String(r.policyCount).padEnd(2)} | Pillars: ${r.fillRate.padStart(6)} (${r.filledCount}/7)`
    );
  });
  ...
  fs.writeFileSync(
    path.join(reportsDir, 'G20_GAP_ANALYSIS.json'),
    JSON.stringify(results, null, 2)
  );

  console.log('-----------------------------------');
  console.log(
    `Report saved to: agent/ccus-ai-agent/governance/reports/G20_GAP_ANALYSIS.json`
  );
