...
async function main() {
  ...
  try {
    ...
    switch (command) {
      ...
      case 'db:compute:maturity': await dbComputeMaturity(SQL); break;
...
async function dbImportIeaLinks(SQL, args = []) {
  const excelPath = args.includes('--excel') ? args[args.indexOf('--excel') + 1] : path.join(__dirname, '../assets/IEA CCUS Projects Database 2025.xlsx');
...
async function dbStats(SQL, args = []) {
  ...
  console.log(`Regulatory Fill Rate: ${(regFillRate * 100).toFixed(1)}% (${filledCells}/${totalCells} cells)`);
  console.log("=======================================");
}
...
async function dbComputeMaturity(SQL) { 
  ...
  execSync("node agent/ccus-ai-agent/logic/maturity-algo.mjs", { stdio: "inherit" }); 
}
