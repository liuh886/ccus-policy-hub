...
async function seed() {
  ...
  const countries = [
    {
      ...
      en: {
        name: 'China',
        summary:
          "China's CCUS framework is rapidly evolving from pilot demonstrations to industrial-scale clusters, guided by the 2024 National Roadmap.",
        regulatory: {
      ...
      zh: {
        name: '中国',
        summary:
          '中国的 CCUS 治理体系正从早期试点向工业集群化阶段跨越，以 2024 版国家技术路线图为战略指引。',
        regulatory: {
    ...
    },
    {
      ...
      en: {
        name: 'United States',
        summary:
          'The US leads global CCUS deployment through robust fiscal incentives (45Q) and a mature EPA-led regulatory framework.',
        regulatory: {
      ...
      zh: {
        name: '美国',
        summary:
          '美国凭借 45Q 强力财政激励与成熟的 EPA 监管框架，在全产业链部署规模上处于全球领先地位。',
        regulatory: {
  ...
  try {
    for (const c of countries) {
      db.run(
        `INSERT OR REPLACE INTO country_profiles (id, region, net_zero_year, capture_2030) VALUES (?, ?, ?, ?)`,
      ...
      ['en', 'zh'].forEach((lang) => {
        const i = c[lang];
        db.run(
          `INSERT OR REPLACE INTO country_i18n (country_id, lang, name, summary, pore_space_rights, liability_transfer, liability_period, financial_assurance, permitting_lead_time, co2_definition, cross_border_rules)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            c.id,
            lang,
            i.name,
            i.summary,
            i.regulatory.pore_space_rights,
            i.regulatory.liability_transfer,
            i.regulatory.liability_period,
            i.regulatory.financial_assurance,
            i.regulatory.permitting_lead_time,
            i.regulatory.co2_definition,
            i.regulatory.cross_border_rules,
          ]
        );
      });
    }
