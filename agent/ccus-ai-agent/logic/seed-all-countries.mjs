...
const MAJOR_COUNTRY_DATA = {
  'United States': {
    en: {
      summary:
        'Global leader in CCUS deployment, driven by the Section 45Q tax credit and established EPA Class VI permitting for geological storage.',
      regulatory: {
        pore_space_rights: 'Private Ownership (Surface owner in most states)',
        liability_transfer:
          'Permitted under specific state/federal frameworks (Class VI)',
    ...
    zh: {
      summary:
        '全球 CCUS 部署的领跑者，主要由 45Q 税收抵免政策和成熟的 EPA Class VI 封存许可体系驱动。',
      regulatory: {
  ...
  China: {
    en: {
      summary:
        'China is rapidly scaling CCUS clusters in key industrial provinces, focused on EOR and new methodologies under the voluntary CCER market.',
      regulatory: {
    ...
    zh: {
      summary:
        '中国正加速在主要工业省份部署 CCUS 集群，重点关注 EOR 以及 CCER 自愿碳市场下的新方法学。',
      regulatory: {
  ...
  Norway: {
    en: {
      summary:
        "Pioneer in offshore CO2 storage through the Longship project and the Northern Lights infrastructure, acting as Europe's central storage hub.",
      regulatory: {
    ...
    zh: {
      summary:
        '通过“长船”计划和“北光”基础设施，挪威成为离岸封存的先驱，扮演着欧洲中央封存枢纽的角色。',
      regulatory: {
  ...
  'European Union': {
    en: {
      summary:
        'Governance centered on the CCS Directive and the Net-Zero Industry Act, creating a harmonized framework for European industrial decarbonization.',
      regulatory: {
    ...
    zh: {
      summary:
        '治理核心为欧盟 CCS 指令与《净零工业法案》，旨在为欧洲工业脱碳建立统一的监管框架。',
      regulatory: {
  ...
  try {
    for (const [id, translations] of Object.entries(countriesData)) {
      ...
      ['en', 'zh'].forEach((lang) => {
        ...
        const summary =
          major?.[lang]?.summary ||
          `National CCUS governance profile for ${name}.`;
        ...
        db.run(
          `INSERT OR REPLACE INTO country_i18n (
          ...
          [
            ...
            name,
            summary,
            reg.pore_space_rights,
            reg.liability_transfer,
            reg.liability_period,
            reg.financial_assurance,
            reg.permitting_lead_time,
            reg.co2_definition,
            reg.cross_border_rules,
          ]
        );
      });
    }
    ...
    console.log(
      `Initialized ${Object.keys(countriesData).length} country profiles.`
    );
  } catch (e) {
    db.run('ROLLBACK');
    console.error(e);
