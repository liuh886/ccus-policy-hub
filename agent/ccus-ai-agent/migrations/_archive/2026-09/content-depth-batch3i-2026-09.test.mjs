...
import {
  ...
  applyContentDepthBatch3I,
} from './content-depth-batch3i-2026-09.mjs';
...
test('content-depth batch 3I enriches five policies and merges ro duplicate (no save)', async () => {
  const db = await openDatabase();
  const summary = applyContentDepthBatch3I(db, { auditDate: '2026-09-09' });
  ...
  if (sourcePresent) {
    ...
    assert.equal(summary.mergedAway.skipped, undefined);
  } else {
  ...
  assert.equal(keptLinks, 3, 'kept record retains all 3 links');

  const aiMarkers = rows(
    db,
    `SELECT COUNT(*) AS n FROM policy_analysis
      WHERE policy_id IN ('sa-circular-carbon-economy-2024','sa-hydrogen-strategy-ccus','ar-rigi-ccus-2024','au-wa-petroleum-amendment-2024','ch-co2-act-ccs-2024')
        AND (evidence LIKE '%[AI-Generated]%' OR label LIKE '%Initial Assessment%' OR evidence LIKE '%No direct incentive found%' OR evidence LIKE '%No specific market rules%' OR evidence LIKE '%Standard reporting required%' OR evidence LIKE '%National program driving massive%' OR evidence LIKE '%Comprehensive integration of NETs%')`,
    []
  )[0].n;
...
test('batch 3I targets score >= 70 on the real audit scorer', async () => {
  const db = await openDatabase();
  applyContentDepthBatch3I(db, { auditDate: '2026-09-09' });
