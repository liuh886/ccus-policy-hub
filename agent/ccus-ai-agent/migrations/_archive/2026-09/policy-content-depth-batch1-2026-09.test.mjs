...
import {
  ...
  applyContentDepthBatch1,
} from './policy-content-depth-batch1-2026-09.mjs';
...
test('content-depth batch 1 enriches three policies in memory (no save)', async () => {
  const db = await openDatabase();
  const summary = applyContentDepthBatch1(db, { auditDate: '2026-09-08' });
...
test('batch 1 targets score >= 70 on the real audit scorer', async () => {
  const db = await openDatabase();
  applyContentDepthBatch1(db, { auditDate: '2026-09-08' });
  const report = buildReport(db, '2026-09-08');
