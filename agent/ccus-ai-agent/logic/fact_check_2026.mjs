...
async function factCheck() {
  ...
  const dbPath =
    '/mnt/GitHub/ccus-policy-hub/agent/ccus-ai-agent/db/ccus_master.sqlite';
  ...
  `);
  console.log(
    `FID 2025: Count = ${fid2025[0].values[0][0]}, Total Capacity = ${fid2025[0].values[0][1].toFixed(2)} Mtpa`
  );
  ...
  `);
  console.log(
    `2025 FID Hub-Linked Capacity %: ${hubPercent[0].values[0][0].toFixed(2)}%`
  );
  ...
  `);
  console.log(
    `Total Cancelled in DB: ${cancelledAudit[0].values[0][0]} projects, Avg Capacity = ${cancelledAudit[0].values[0][1].toFixed(2)} Mtpa`
  );
  ...
  // 注意：这个数字可能包含状态更新的项目
  console.log(
    `Facilities processed/touched in March 2026 refresh: ${new2026[0].values[0][0]}`
  );
}
