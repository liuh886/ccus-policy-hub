...
本手册是 `agent/ccus-ai-agent/governance/0_GOVERNANCE.md` 的可执行实现，约束目标是让治理流程在当前仓库中可重复、可审计、可导出。

## 0. 权限与前提

- SSOT 只有一个：`agent/ccus-ai-agent/db/ccus_master.sqlite`。
- 执行前必须读取：`agent/ccus-ai-agent/governance/0_GOVERNANCE.md`、`agent/ccus-ai-agent/governance/OPERATING_RULES_AGENT.md`。
...
2. `pnpm manage db:import:iea:links --excel "agent/ccus-ai-agent/governance/IEA CCUS Projects Database 2025.xlsx"`（可选但推荐）
...
- 审计心跳：`agent/ccus-ai-agent/governance/reports/governance_heartbeat.json`
- 数据概览：`agent/ccus-ai-agent/governance/reports/stats.json`
- 摘要报告：`agent/ccus-ai-agent/governance/reports/data_integrity_summary.md`
- 阈值配置：`agent/ccus-ai-agent/governance/audit_thresholds.json`
...
- 报错 Excel 缺失：检查 `agent/ccus-ai-agent/governance/IEA CCUS Projects Database 2025.xlsx` 是否存在，或显式传入 `--excel`。
