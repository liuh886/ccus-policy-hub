...
import {
  ...
  applyContentDepthBatch3E,
} from './content-depth-batch3e-2026-09.mjs';
...
test('content-depth batch 3E enriches five MENA policies in memory (no save)', async () => {
  const db = await openDatabase();
  const summary = applyContentDepthBatch3E(db, { auditDate: '2026-09-09' });
  ...
  const aiMarkers = rows(
    ...
    `SELECT COUNT(*) AS n FROM policy_analysis
      WHERE policy_id IN ('ae-carbon-strategy','ae-federal-climate-law-2024','kw-kpc-net-zero-2024','om-ccus-roadmap-2024','qa-national-climate-strategy-2024')
        AND (evidence LIKE '%[AI-Generated]%' OR label LIKE '%Initial Assessment%' OR evidence LIKE '%No direct incentive found%' OR evidence LIKE '%No specific market rules%' OR evidence LIKE '%Standard reporting required%' OR evidence LIKE '%Strong mandatory MRV and federal climate legislation%' OR evidence LIKE '%Strategic roadmap for KPC decarbonization%' OR evidence LIKE '%Strong alignment with National Net Zero 2050 target%' OR evidence LIKE '%Highly integrated into massive national LNG infrastructure%')`,
    []
  )[0].n;
...
test('batch 3E targets score >= 70 on the real audit scorer', async () => {
  const db = await openDatabase();
  applyContentDepthBatch3E(db, { auditDate: '2026-09-09' });
