...
async function dbImportIeaLinks(SQL, args = []) {
  const excelPath = args.includes('--excel')
    ...
    : path.join(__dirname, '../assets/IEA CCUS Projects Database 2025.xlsx');
...
async function dbExportMd(SQL) {
  ...
  if (!auditPass || auditPass.value !== 'true') {
    if (args.includes('--force-export')) {
      console.warn(
        'WARNING: Export proceeding without full audit pass (--force-export).'
      );
    } else {
      throw new Error(
        'Gate B2: Export blocked. Last audit did not pass. Run "db:audit:deep" first, or use --force-export to override.'
      );
    }
  }
  ...
  const translate = (key, dom, lang) => {
    ...
    if (dom === 'country') {
      for (const [a, c] of Object.entries(dict.countries))
        if (c === key && /[\u4e00-\u9fa5]/.test(a)) return a;
    }
...
async function dbImportMdReverse(SQL, argv = []) {
  ...
  ['en', 'zh'].forEach((l) =>
    processDir(
      path.join(__dirname, '../../../src/content/policies', l),
      'policy'
    )
  );
  ['en', 'zh'].forEach((l) =>
    processDir(
      path.join(__dirname, '../../../src/content/facilities', l),
      'facility'
    )
  );
