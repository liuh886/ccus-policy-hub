...
import {
  MIGRATION_ID,
  NEW_POLICIES,
  applyCoveragePhase2A,
} from './coverage-phase2a-2026-09.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const DB_PATH = path.resolve(__dirname, '../../db/ccus_master.sqlite');

const TARGETS = [
  'cn-ccus-national-standards-2026',
  'cn-co2-transport-status-2025',
];
...
test('coverage phase 2A inserts two China records in memory (no save)', async () => {
  const db = await openDatabase();
  const summary = applyCoveragePhase2A(db, { auditDate: '2026-09-09' });

  assert.equal(summary.migrationId, MIGRATION_ID);
  assert.deepEqual(
    summary.insertedPolicies.concat(summary.skippedPolicies).sort(),
    NEW_POLICIES.map((entry) => entry.core.id).sort()
  );

  for (const id of TARGETS) {
    const core = rows(db, 'SELECT * FROM policies WHERE id = ?', [id])[0];
    assert.ok(core, `${id} exists`);
    assert.equal(core.country, 'China');
    assert.equal(core.review_status, 'draft');
    assert.ok(core.url && core.url.startsWith('http'), `${id} has real url`);
    const locales = rows(
      db,
...
test('phase 2A targets score >= 70 on the real audit scorer', async () => {
  const db = await openDatabase();
  applyCoveragePhase2A(db, { auditDate: '2026-09-09' });
